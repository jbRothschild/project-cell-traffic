...
class Bacteria():
    ...
    def splitting_velocities(self, newp1, newp2):
        ...
        vec2newCom3d = np.pad(newCenter - self.center, [(1, 0)]
                                        , mode='constant', constant_values=0)

        # 3d vector from old CoM to new p1 (0,x1,y1)
        vec2newp13d = np.pad(newp1 - self.center, [(1, 0)]
                                        , mode='constant', constant_values=0)
        ...
        newComVel3d = np.pad(self.comVel, [(1, 0)], mode='constant'
                        , constant_values=0) + np.cross(angVel3d, vec2newCom3d)


        # V_newp1 = V_com + w x r = newp1Vel3d (0,vx1,vy1)
        newp1Vel3d = np.pad(self.comVel, [(1, 0)], mode='constant'
                        , constant_values=0) + np.cross(angVel3d, vec2newp13d)
